...
class YoLov5TRT(object):
    ...
    def infer(self,input_image,origin_h,origin_w):
        ...
        for i in range(self.batch_size):
            batch_input_image[i] = input_image[i]
        ...
        for i in range(self.batch_size):
            ...
            if result_boxes.shape[0] == 0:
                # 没有检测到目标，跳过这条结果，不append
                continue
            
            all_results.append({
                'boxes': result_boxes,
                'conf': result_scores,
                'classid': categories[int(result_classid[0])],
            })
...
def infer_once(yolo_model, output_array, output_lock, input_shape, resized_shape):
    resize_h = resized_shape[1]
    resize_w = resized_shape[2]
    output_np = np.frombuffer(output_array, dtype=np.float32).reshape(3, resize_h, resize_w)

    with output_lock:
        input_data = [output_np.copy()]  # batch_size=1，放到列表里

    results, use_time = yolo_model.infer(input_data, input_shape[0], input_shape[1])
    ...
    for k, result in enumerate(results):
        ...
        if pred[k].conf < result['conf'][0]:
            ...
            pred[k].classid = result['classid']
            print(pred[k].boxes)
            print(f"conf:{pred[k].conf}")
            print(f"classid:{pred[k].classid}")
...
if __name__ == "__main__":
    ...
    # 用 Array 创建共享内存（对应 c_uint8 和 c_float）
    ...
    try:
        while True:
            t1 = time.time()
            results = infer_once(model, output_array, output_lock, input_shape, resized_shape)
            visualize(input_array, input_lock, results, input_shape)
            t2 = time.time()
            fps = 1/(t2-t1)
