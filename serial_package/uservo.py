...
class UartServoManager:
	...
	ADDRESS_STATUS = 5
	def __init__(self, uart, is_scan_servo=True, srv_num=2, mean_dps=100, is_debug=False):
		self.is_debug = is_debug
		self.uart = uart
	...
	def update(self, is_empty_buffer=False, wait_response=False, timeout=0.05):
		'''舵机管理器的定时器回调函数
		Args:
			is_empty_buffer (bool): 是否清空缓冲区
		...
		while True:
			try:
				...
				if bytes_waiting:
					...
					if wait_response:
						# 如果超过空闲阈值，检查数据完整性
						if (now_ns - last_data_time_ns) > idle_threshold_ns:
							if len(buffer) > 0:  # 确保至少收到了一些数据
								break
					else:
						break
					...
					if (now_ns - t_start_ns) > timeout_ns:
						if self.is_debug:
							logging.warning('Serial communication timeout')
						break
			except serial.SerialException as e:
